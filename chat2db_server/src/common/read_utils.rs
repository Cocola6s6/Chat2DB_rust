...
impl ReadUtils {
    ...
    pub fn get_openai_key() -> String {
        let openai_key = Self::read_input("Enter OPENAI_KEY:").unwrap_or_default();

        // 如果不输入从配置文件中读取
        let env_key = std::env::var("OPENAI_KEY").unwrap();
        let openai_key = if openai_key.is_empty() {
            env_key
        } else {
            openai_key
        };
    ...
    pub fn get_db_url() -> String {
        let db_url = Self::read_input("Enter DB_URL:").unwrap_or_default();
    ...
    pub fn get_db_ns() -> String {
        let db_ns = Self::read_input("Enter DB_NS:").unwrap_or_default();
    ...
    pub fn get_sql() -> String {
        let sql = Self::read_input("Enter sql:").unwrap_or_default();
